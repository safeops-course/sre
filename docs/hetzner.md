...
3. Confirm the GitHub Environment `hetzner` exists **before** any workflow runs (Settings ->
   Environments): required reviewer = the platform owner, deployment branches = `main` only.
   The apply, destroy and e2e jobs use `environment: hetzner`, and GitHub creates a missing
